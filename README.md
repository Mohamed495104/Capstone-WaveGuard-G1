...
| Layer | Technologies |
|-------|-------------|
| **Frontend** | Next.js 16 (App Router), React 19, Material UI 7, next-pwa, Recharts, Framer Motion |
...
- **Frontend:** Hosted on Vercel (Next.js project root at `frontend/`)
- **Backend:** Hosted on DigitalOcean App Platform (source directory `backend/`)
- **Database:** MongoDB Atlas (shared cluster)
- Update `NEXT_PUBLIC_API_URL` to your DigitalOcean backend URL and set `FRONTEND_URL` on the backend to your Vercel domain. See [Deployment](./docs/DEPLOYMENT.md) for detailed steps.
...
- Installable PWA with manifest, maskable icons, and standalone display mode
- Service worker generated by **next-pwa** with a branded `/offline.html` fallback
- Runtime caching for `/api/challenges`, `/api/profile`, `/api/achievements` plus 30-day image caching
- IndexedDB-backed request cache to reduce duplicate calls and keep previously viewed API data available across sessions
